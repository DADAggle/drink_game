...
import { checkEnding, stepState } from '../state/Reducers';
import { Panel } from '../ui/Panel';
import { Toast } from '../ui/Toast';
import { ActionSystem } from '../systems/ActionSystem';
import { NpcSystem, DialogQuestion } from '../systems/NpcSystem';
import { DrunkEffectSystem } from '../systems/DrunkEffectSystem';
...
type PhoneView = 'closed' | 'menu' | 'video' | 'wechat' | 'moments';

export class PartyScene implements Scene {
  private readonly action = new ActionSystem();
  private readonly npc = new NpcSystem();
  private readonly drunk = new DrunkEffectSystem();
  private readonly toast = new Toast();
  private readonly assets = new AssetLoader();

  private doorRect!: Rect;
  private drinkRect!: Rect;
  private phoneRect!: Rect;
  private pretendRect!: Rect;
  private npcRect!: Rect;

  private tapQueue: TapTask[] = [];
  private dialog: DialogQuestion | null = null;
  private dialogRects: Rect[] = [];

  private phoneView: PhoneView = 'closed';
  private phoneVolume = 3;
  private phoneCloseRect!: Rect;
  private phoneMenuRects!: { video: Rect; wechat: Rect; moments: Rect };
  private videoVolumeRects: Rect[] = [];
  private videoPlayRect!: Rect;
  private videoSliderRect!: Rect;
  private draggingVolumeSlider = false;
  private spamTimer = 0;
  private spamTaps = 0;
  ...
  private flushTapQueue(now: number): void {
    if (this.tapQueue.length === 0) return;
  ...
  private handleTap(rawX: number, rawY: number): void {
    const state = this.shared.getState();
  ...
  private handlePhoneTap(x: number, y: number): void {
    const state = this.shared.getState();
    ...
    if (this.phoneView === 'menu') {
      ...
      if (this.hitRect(this.phoneMenuRects.wechat, x, y) && canWechat) {
        ...
        this.spamTimer = GAME_CONFIG.phoneMiniGame.spamDurationSec;
        this.spamTaps = 0;
        return;
      }
      if (this.hitRect(this.phoneMenuRects.wechat, x, y) && !canWechat) {
  ...
  private renderBackground(ctx: CanvasRenderingContext2D): void {
    ...
    Panel.draw(ctx, { x: 0, y: 0, w: this.shared.width, h: this.shared.height }, '#000000', 0.38);
  }

  private renderMainButtons(ctx: CanvasRenderingContext2D): void {
    const state = this.shared.getState();
  ...
  private renderTopHud(ctx: CanvasRenderingContext2D): void {
    const state = this.shared.getState();
    Panel.draw(ctx, { x: 12, y: 12, w: 272, h: 156 }, '#111111');
    ...
    ctx.fillText(GAME_CONFIG.title, 206, 16);
    if (state.cooldowns.pretend > 0) {
      ctx.fillText(`假装CD: ${state.cooldowns.pretend.toFixed(1)}s`, 206, 34);
    }
  }

  private renderBackgroundNpc(ctx: CanvasRenderingContext2D, tol: number): void {
    let face = ':)';
    let key = 'npcBgNormal';
    if (tol < 60 && tol >= 30) {
      face = ':|';
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      Panel.draw(ctx, { x: rect.x - 2, y: rect.y - 2, w: rect.w + 4, h: rect.h + 4 }, '#0b0b0b', 0.36);
  ...
  private renderNpc(ctx: CanvasRenderingContext2D): void {
    Panel.draw(
      ctx,
      { x: this.npcRect.x - 4, y: this.npcRect.y - 4, w: this.npcRect.w + 8, h: this.npcRect.h + 8 },
      '#0b0b0b',
      0.4
    );
  ...
  private renderPlayer(ctx: CanvasRenderingContext2D, alc: number): void {
    const h = this.shared.height;
    const x = 18;
    const y = h - 130;
    Panel.draw(ctx, { x, y, w: 150, h: 102 }, '#121212', 0.85);
  ...
  private renderDialog(ctx: CanvasRenderingContext2D, alc: number): void {
    if (!this.dialog) {
      this.dialogRects = [];
      return;
    }
    ...
    Panel.draw(ctx, { x, y, w, h: 154 }, '#1e1e2f', 0.92);
    ctx.fillStyle = '#ffffff';
    ...
    for (let i = 0; i < d.options.length; i += 1) {
      ...
      Panel.draw(ctx, rect, '#3b4f6a', 0.95);
      ctx.fillStyle = '#ffffff';
      ctx.fillText(this.npc.garble(`${i + 1}. ${d.options[i]}`, alc), rect.x + 8, rect.y + 20);
    }
  }

  private renderQte(ctx: CanvasRenderingContext2D): void {
    if (!this.npc.qte.active) return;
    Panel.draw(ctx, { x: 24, y: 130, w: this.shared.width - 48, h: 74 }, '#5a2f2f', 0.9);
    ctx.fillStyle = '#ffffff';
    ctx.fillText('NPC劝酒中！连点屏幕拒绝', 36, 158);
    ctx.fillText(
      `进度: ${this.npc.qte.taps}/${this.npc.qte.need}  剩余: ${this.npc.qte.remaining.toFixed(1)}s`,
      36,
      183
    );
  }

  private renderPhoneOverlay(ctx: CanvasRenderingContext2D): void {
    const w = this.shared.width;
    const h = this.shared.height;
    const x = 18;
    const y = h * 0.14;
    ...
    Panel.draw(ctx, { x: 0, y: 0, w, h }, '#000000', 0.35);
    Panel.draw(ctx, { x, y, w: pw, h: ph }, '#0f1622', 0.98);
    ...
    ctx.font = '13px sans-serif';
    ctx.fillText(
      `酒精:${this.levelText(state.stats.alc / 100, true)}  容忍:${this.levelText(state.stats.tol / 100, false)}  音量:${this.phoneVolume}档`,
      x + 14,
      y + 54
    );
    ...
    if (this.phoneView === 'video') {
      ...
      for (let i = 0; i < this.videoVolumeRects.length; i += 1) {
        ...
        Panel.draw(ctx, rect, selected ? '#4a88cc' : '#304050', 0.95);
        ctx.fillStyle = '#ffffff';
    ...
    if (this.phoneView === 'wechat' || this.phoneView === 'moments') {
      ...
      Panel.draw(ctx, { x: x + 18, y: y + 250, w: pw - 36, h: ph - 290 }, '#1a2738', 0.9);
      ctx.fillStyle = '#8fd7ff';
      ctx.fillText('在这里连点', x + pw / 2 - 28, y + ph / 2);
    }
  }

  private withDrift(rect: Rect, seed: number): Rect {
    const state = this.shared.getState();
    if (!state.drunkFlags.flyButtons) return rect;
    const drift = this.drunk.buttonDrift(seed, state.stats.alc);
    return { x: rect.x + drift.dx, y: rect.y + drift.dy, w: rect.w, h: rect.h };
  }

  private hitRect(rect: Rect, x: number, y: number): boolean {
    return x >= rect.x && x <= rect.x + rect.w && y >= rect.y && y <= rect.y + rect.h;
  }

  private drawImageButton(
  ...
  private renderVolumeSlider(ctx: CanvasRenderingContext2D): void {
    const r = this.videoSliderRect;
    Panel.draw(ctx, r, '#233245', 0.95);
    ...
    Panel.draw(ctx, { x: knobX - 8, y: r.y - 6, w: 16, h: r.h + 12 }, '#5ab1ff', 1);
  }
  ...
  private renderStatBar(
    ...
    Panel.draw(ctx, { x: barX, y: barY, w: barW, h: barH }, '#283342', 1);
  ...
  private renderDoorTimer(ctx: CanvasRenderingContext2D, timeLeft: number, doorRect: Rect): void {
    const timerRect = { x: doorRect.x, y: doorRect.y + doorRect.h + 6, w: doorRect.w, h: 26 };
    Panel.draw(ctx, timerRect, '#0b0b0b', 0.72);
    ctx.strokeStyle = '#d0d0d0';
  ...
  private levelColor(rate: number, dangerHigh: boolean): string {
    if (dangerHigh) {
      if (rate < 0.35) return '#2ab36c';
      if (rate < 0.7) return '#e0b13d';
      return '#d24b4b';
    }
    if (rate < 0.35) return '#d24b4b';
    if (rate < 0.7) return '#e0b13d';
    return '#2ab36c';
  }

  private levelText(rate: number, dangerHigh: boolean): string {
    if (dangerHigh) {
      if (rate < 0.35) return '低';
  ...
  private formatCountdown(timeLeft: number): string {
  ...
  private setVolumeBySliderX(x: number): void {
    const r = this.videoSliderRect;
  ...
  private drawImageContain(ctx: CanvasRenderingContext2D, image: any, rect: Rect): void {
