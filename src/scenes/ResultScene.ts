...
import { EndingType, GameState } from '../state/GameState';
import { endingText, titleByShame } from '../state/Reducers';
import { generateCallBomb, generateMoments, MomentItem } from '../systems/ResultContentGenerator';
...
export class ResultScene implements Scene {
  ...
  private ending: EndingType = 'timeup';
  ...
  constructor(private readonly shared: SharedContext) {
    this.restartBtn = new Button(
      { x: this.shared.width / 2 - 100, y: this.shared.height - 98, w: 200, h: 56 },
      '你还想再喝吗',
      true,
      '#3f3f3f'
    );
  ...
  onEnter(payload?: unknown): void {
    const state = (payload as { state: GameState } | undefined)?.state ?? this.shared.getState();
    ...
    this.title = titleByShame(state.stats.shame);
    ...
    this.calls = generateCallBomb(state.counters.harass, seed);
    this.moments = generateMoments(state.counters.post, seed + 97);
  }
  ...
  render(ctx: CanvasRenderingContext2D): void {
    ...
    const localPaper: PaperRect = { x: 0, y: 0, w: paper.w, h: paper.h };
  ...
  onTouchMove(_x: number, y: number): void {
    if (!this.dragging || this.maxScroll <= 0 || this.revealTime < 0.5) return;
  ...
  private paperRect(w: number, h: number): PaperRect {
    const pw = Math.min(430, w - 26);
    const ph = h - 130;
    return { x: (w - pw) / 2, y: h / 2 - ph / 2 - 10, w: pw, h: ph };
  }

  private paperTransform(w: number, h: number): { cx: number; cy: number; rot: number } {
    const paper = this.paperRect(w, h);
  ...
  private drawPaperShadow(ctx: CanvasRenderingContext2D, paper: PaperRect): void {
    ctx.fillStyle = 'rgba(0,0,0,0.34)';
    ctx.fillRect(paper.x + 6, paper.y + 8, paper.w, paper.h);
  }

  private drawPaper(ctx: CanvasRenderingContext2D, paper: PaperRect): void {
  ...
  private drawMasthead(ctx: CanvasRenderingContext2D, paper: PaperRect): void {
    const x = paper.x + 16;
    let y = paper.y + 34;

    ctx.fillStyle = '#121212';
    ctx.font = 'bold 28px sans-serif';
    ctx.fillText(GAME_CONFIG.title, x, y);
  ...
  private drawScrollableContent(ctx: CanvasRenderingContext2D, viewport: { x: number; y: number; w: number; h: number }): void {
    ctx.save();
    ctx.beginPath();
    ...
    this.section(ctx, 0.24, () => {
      ...
      ctx.fillText(endingText(this.ending), x + 10, y + 8);
      y += 34;
      ctx.fillStyle = '#111111';
  ...
  private drawFooterMeta(ctx: CanvasRenderingContext2D, paper: PaperRect): void {
    const y = paper.h - 74;
  ...
  private drawScrollHint(ctx: CanvasRenderingContext2D, viewport: { x: number; y: number; w: number; h: number }): void {
  ...
  private drawCallBubbles(ctx: CanvasRenderingContext2D, x: number, y: number, width: number): number {
  ...
  private drawMoments(ctx: CanvasRenderingContext2D, x: number, y: number, width: number): number {
    const cardH = 64;
    for (let i = 0; i < this.moments.length; i += 1) {
      ...
      const line = this.wrapText(m.text, 28)[0] ?? m.text;
  ...
  private measureContentHeight(width: number): number {
    let y = 0;
  ...
  private section(ctx: CanvasRenderingContext2D, delay: number, draw: () => void): void {
